...
export const getOneRamadan = (id1, id2, id3) => {
    console.log(id1)
    return axios.get(
        `http://18.217.131.163:8989/ramadan/?englishMonth=${id2}&englishYear=${id3}&englishDay=${id1}`
    );
};
export const updateRamadan = (id, addAdan) => {
    const token = JSON.parse(localStorage.getItem('jwt'))
    return axios.put(`http://18.217.131.163:8989/ramadan/${id}`, addAdan, {
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
        }
...
export const deleteRamadan= (id) => {
    console.log(id)
    const token = JSON.parse(localStorage.getItem('jwt'))
    return axios.delete(`http://18.217.131.163:8989/ramadan/${id}`, {
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
        }
